...
/// Struct defining an interface, generated by compiling a `.api` file
///
...
impl Interface {
    ...
    fn populate(&mut self, methods: &Vec<InterfaceMethodDec>) {
        ...
        for m in methods {
            match m.t {
                InterfaceMethodType::Import => self.add_import(
                    MethodImport {
                        name: m.name.clone(),
                        params: m.params.clone().into_iter().map(|(t, _)| t).collect(),
                        return_type: m.ret.clone(),
                    },
                    MethodId(id),
                ),
                InterfaceMethodType::Export => todo!(),
            }
            id += 1;
        }
    }
...
pub fn compile_api(raw: &str) -> anyhow::Result<Interface> {
